...
class LocalQuantumLayer(MessagePassing):
    ...
    def apply_local_unitary_evolution(self, x_complex, edge_index, U_small):
        """在局部邻域应用酉演化"""
        ...
        for node_idx in unique_nodes:
            ...
            neighbors = torch.unique(torch.cat([
                row[all_mask], col[all_mask]
            ]))


            # 限制邻居数量以匹配酉矩阵维度
            if len(neighbors) > unitary_dim:
                neighbors = neighbors[:unitary_dim]

            neighbor_features = x_complex[neighbors]  # [num_neighbors, output_dim]
            # 对每个特征维度应用酉演化
            for dim in range(output_dim):
                # 创建填充特征向量
                padded_features = torch.zeros(unitary_dim, dtype=torch.cfloat, device=device)
                padded_features[:len(neighbors)] = neighbor_features[:, dim]

                # 应用酉演化
                try:
                    evolved_padded = U_small @ padded_features
                    evolved_x[neighbors, dim] = evolved_padded[:len(neighbors)]
...
class QuantumGraphConvNet(nn.Module):
    ...
    def forward(self, data):
        ...
        # print(f"Before pool1: x={x.shape}, edge_index={edge_index.shape}")
        x, edge_index, _, batch, _, _ = self.pool1(x, edge_index, batch=batch)
        # print(f"After pool1: x={x.shape}, edge_index={edge_index.shape}")
        x = F.relu(self.gcn2(x, edge_index))
        x, edge_index, _, batch, _, _ = self.pool2(x, edge_index, batch=batch)
        # print(f"After pool2: x={x.shape}, edge_index={edge_index.shape}")
        # ----- 初始化复数输入 -----
